...
import com.boseongcho.in4goback.jwt.JwtAccessDeniedHandler;
import com.boseongcho.in4goback.jwt.JwtAuthenticationEntryPoint;
...
public class SecurityConfig {
    ...
    {
        return (web) -> web.ignoring().requestMatchers("/imgs/**");
    }
    ...
    public SecurityFilterChain filterChain(HttpSecurity http) throws Exception {
        ...
        http.csrf(csrf -> csrf.disable())
                .exceptionHandling(exception ->
                        ...
                        exception.authenticationEntryPoint(jwtAuthenticationEntryPoint) // 유효한 자격 증명 없을 시(401)
                                .accessDeniedHandler(jwtAccessDeniedHandler)  // 필요한 권한 없이 접근 시 (403)
                )
                .authorizeHttpRequests((authorizeHttp) ->
                        authorizeHttp.requestMatchers("/").authenticated()
                                .requestMatchers(HttpMethod.OPTIONS, "/**").permitAll()
                                .requestMatchers("/api/v1/dailyWorker/**").permitAll()
                                .requestMatchers("/api/v1/approval/submit").hasRole("INSA1")
                                .requestMatchers("/api/v1/approval/").hasAnyRole("DESIGN1", "MARKETING1")
                                .requestMatchers("/api/v1/marketing/management").hasRole("MARKETING2")
...
//        configuration.setAllowedOrigins(Arrays.asList("http://localhost:3001")); //이 origin을 허용하겠다/
        ...
        configuration.setAllowedHeaders(Arrays.asList("Access-Control-Allow-Origin", "Content-type" //여기있는 method는
                // 허용하겠다.
