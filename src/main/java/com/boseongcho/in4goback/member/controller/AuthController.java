...
public class AuthController {
    ...
    /* @RequestBody를 통해 넘어온 Json 문자열을 파싱해서 MemberDTO 속성으로 매핑해 객체로 받아낸다.(회원 아이디, 비밀번호) */

    @Operation(summary = "로그인 요청", description = "로그인 및 인증이 진행됩니다.", tags = {"AuthController"})
    @PostMapping("/login")
    public ResponseEntity<ResponseDTO> loging(@RequestBody MemberDTO memberDTO){
        ...
        화면단의 React인 곳으로 가면 Store에 해당 리듀서가 관리하는 state값이 된다.
        * */
    }
}
